...
func (w *AvsWriter) SendNewPriceUpdate(ctx context.Context, feedName string) (cstaskmanager.IIncredibleSquaringTaskManagerPriceUpdateTask, uint32, error) {
	...
	//threshold, _ := w.AvsContractBindings.TaskManager.DEFAULTQUORUMTHRESHOLD(&bind.CallOpts{})
	...
	tx, err := w.AvsContractBindings.TaskManager.RequestPriceFeedUpdate(txOpts, feedName, uint32(sdktypes.QuorumThresholdPercentage(33)), sdktypes.QuorumNums{0}.UnderlyingType(), uint8(2))
	if err != nil {
